...
local function load_private_keys()
  local content = os_getenv("KONG_FACEBOOK_OAUTH_JWT_SIGNER_PRIVATE_KEYS")
  if content == nil or err then
      ngx_log(ngx_ERR, "Could not read KONG_FACEBOOK_OAUTH_JWT_SIGNER_PRIVATE_KEYS env var.")
      return nil, tostring(err)
  end

  local pkeys = json.decode(content)
  if not pkeys then
    ngx_log(ngx_ERR, "Could not get 'keys' object from KONG_FACEBOOK_OAUTH_JWT_SIGNER_PRIVATE_KEYS env var" )
    return nil, "Could not get 'keys' object from KONG_FACEBOOK_OAUTH_JWT_SIGNER_PRIVATE_KEYS env var"
  end
...
local env_client_id = os_getenv("KONG_FACEBOOK_OAUTH_JWT_SIGNER_CLIENT_ID")
local env_client_secret = os_getenv("KONG_FACEBOOK_OAUTH_JWT_SIGNER_CLIENT_SECRET")
...
function plugin:access(conf)
    ...
    if not client_id then
        kong.response.exit(500, "configure client_id plugin param or env var KONG_FACEBOOK_OAUTH_JWT_SIGNER_CLIENT_ID")
    end
    if not client_secret then
        kong.response.exit(500, "configure client_secret plugin param or env var KONG_FACEBOOK_OAUTH_JWT_SIGNER_CLIENT_SECRET")
    end
